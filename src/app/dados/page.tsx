  ...
  return (
    <main className="max-w-3xl mx-auto px-4 py-6">
      ...
      <div className="mb-6">
        ...
        <h2 className="text-xl font-bold text-texto">Gerenciar dados</h2>
        <p className="text-sm text-texto-suave mt-0.5">Visão geral do banco e opções de limpeza para testes</p>
      </div>
      ...
      {indisponivel && !loadingStats && (
        <div className="bg-risco-50 border border-risco-200 rounded-md p-4 mb-6 text-sm">
          <p className="text-risco-700 font-semibold">Dados indisponíveis</p>
          <p className="text-risco-700 text-xs mt-1 leading-relaxed">{indisponivel}</p>
          <button
            onClick={() => {
              setLoadingStats(true);
              carregarStats();
            }}
            className="mt-3 text-xs font-semibold px-3 py-1.5 rounded-lg bg-superficie border border-risco-200 text-risco-700 hover:bg-risco-100 transition-colors"
          >
            Tentar de novo
          </button>
        </div>
      ...
      <div className={`grid grid-cols-2 md:grid-cols-3 gap-3 mb-6 ${indisponivel && !loadingStats ? 'hidden' : ''}`}>
        {loadingStats ? (
          ...
          <p className="text-sm text-texto-fraco col-span-3">Erro ao carregar dados.</p>
        )}
      </div>

      {/* Sucesso da exclusão */}
      {mensagem && (
        <div className="bg-marca-50 border border-marca-200 rounded-md p-3.5 flex gap-2.5 text-sm text-marca-800 mb-6">
          <span>✅</span>
          <span>{mensagem}</span>
        </div>
      )}
      ...
      {erroExclusao && (
        <div className="bg-risco-50 border border-risco-200 rounded-md p-3.5 flex gap-2.5 text-sm mb-6">
          <span className="shrink-0">❌</span>
          <div>
            <p className="text-risco-700 font-semibold">A exclusão não foi concluída</p>
            <p className="text-risco-700 text-xs mt-0.5 leading-relaxed">{erroExclusao}</p>
          </div>
        </div>
      )}
      ...
      >
        <div className="px-5 py-4 border-b border-borda bg-superficie-sutil">
          <h3 className="text-sm font-semibold text-texto">Zona de perigo</h3>
          <p className="text-xs text-texto-fraco mt-0.5">Ações irreversíveis — não há desfazer</p>
        </div>
...
function StatCard({
  ...
  return (
    <div className="bg-superficie border border-borda rounded-md p-4 ">
      <p className="text-xs font-medium text-texto-suave uppercase tracking-wide mb-1">{label}</p>
      <p
      ...
      {hint && <p className="text-xs text-texto-fraco mt-1 leading-tight">{hint}</p>}
    </div>
  );
}
    ...
    <div className="px-5 py-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="text-xs text-texto-fraco mt-0.5 leading-relaxed">{descricao}</p>
        </div>
        ...
        {!confirmando ? (
          ...
          <div className="flex items-center gap-2 shrink-0 flex-wrap">
            <span className="text-xs font-semibold text-risco-700">Tem certeza?</span>
            <button
              onClick={onConfirmar}
              disabled={deletando}
